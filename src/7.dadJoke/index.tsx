...
type FetchStatus = 'loading' | 'error' | 'data'
...
type FetchActions = LoadingAction | SuccessAction | ErrorAction

function fetchReducer(state: FetchState, action: FetchActions): FetchState {
  switch (action.type) {
    case 'loading':
      return { ...state, state: 'loading', data: null, error: null }
    case 'data':
      return { ...state, state: 'data', data: action.data, error: null }
    case 'error':
      return { ...state, state: 'error', data: null, error: action.error }
    default:
      return state
  }
}
